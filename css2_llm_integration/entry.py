...
async def main():
    ...
    with (input_col):
        ...
        active_adaptor = model_choices[(selected_model if selected_model is not None else "ChatGPT API")]()
        ...
        with st.form(key="batch_prompts"):
            ...
            if run:
                ...
                fake_history: typing.List[HistoryMessage] = []

                session = ChatSession(model=st.session_state.active_adaptor, db=db_conn,
                                      history=fake_history)
                batch_response = []
                for prompt_in in prompts:
                    run = []
                    fake_history.clear()

                    if type(prompt_in) is str:
                        run.append(prompt_in)
                    elif type(prompt_in) is list:
                        run = prompt_in
